...
2. **Check the version**: this plugin's workflows need **4.1.4 or newer** (`fluree auth token` ships in 4.1.4; the `fluree model` family in 4.1.3). If the version is below the floor, first run `which -a fluree` — a newer install is often *shadowed* by an older one earlier on PATH (e.g. `~/.cargo/bin` ahead of `~/bin`), and the fix is PATH order, not a reinstall. Only if no newer binary exists anywhere, recommend upgrading via the user's original install method (infer it from the binary's directory) before continuing.
