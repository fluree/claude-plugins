...
- **An MCP server entry** that exposes the binary's embedded docs (`docs_search`, `docs_get`, `docs_examples`, `docs_tree`) to Claude automatically. If `fluree` isn't installed yet you'll see a failed `fluree-docs` server in `/mcp` and the Errors tab — that's expected, not a broken plugin: everything else keeps working, and `/fluree-cli:setup` walks you through installing (after which a restart clears it).
...
Two mirroring relationships are intentional and should be reviewed together when either side changes:

- **The skill ↔ the binary's `docs/ai/claude-code.md`.** A skill has to be useful before the binary is installed, so the durable doctrine (safety rails, device-flow choreography, machine-output map) appears in both. The binary's page wins on the installed version's *command surface*; this skill wins on agent *behavior*. Neither is machine-checked prose — when a CLI release changes behavior, check both.
- **The skill's remote workflow ↔ the Fluree AI Build-an-App prompt.** By the settled division of labor the prompt carries what only the stack knows (origin, spaceId) and is expected to shrink toward that bootstrap over time, while durable procedure lives here and in the stack's served `building-apps.md`. During the transition the same choreography exists in both places on purpose.
